 ...
 * the chat drawer.
 */
const UnreadTopicItem = memo<UnreadTopicItemProps>(({ topic, onReload }) => {
  ...
  // Reply in place: send straight to the topic over the gateway so the agent
  // picks up where it left off and keeps running server-side, no navigation.
  const submitFollowUp = useCallback(
    async (text: string) => {
      if (!agentId) return;
      markRead();
      await sendMessage({
        context: { agentId, isolatedTopic: true, scope: 'main', topicId: topic.id },
        forceRuntime: 'gateway',
        message: text,
      });
      setReplying(false);
      // The run flips the topic to `running`, but that status write is a
      // fire-and-forget round trip that lands just after the send resolves —
      // refetch on the next tick so the list catches the flip instead of racing
      // it and reading the row still `unread`.
      if (onReload) setTimeout(onReload, 800);
    },
    [agentId, markRead, onReload, sendMessage, topic.id],
  );
 ...
 */
const UnreadTopicList = memo<UnreadTopicListProps>(({ topics, onReload }) => (
  <Flexbox className={styles.list}>
    {topics.map((topic) => (
      <UnreadTopicItem key={topic.id} topic={topic} onReload={onReload} />
    ))}
  </Flexbox>
));
